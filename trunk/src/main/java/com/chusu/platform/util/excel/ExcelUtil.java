package com.chusu.platform.util.excel;
...
//    orders.setDepartment_name("");
    orders.setTypeId(i);
    orders.setNumber(100);
...
//
//    String s = StringUtil.splicts("D:\\apache-tomcat-7.0.73\\webapps\\hos\\");
//    // Excel 的表头
...
//    }
//    String urs = "poi_tests";
//    //创建一个文件
//    File file = new File(""+s+"//assets//excel//"+urs+".xls");
//
//
//    try {
//      file.createNewFile();
...
//    }
   ...
   * @return
   */
  public static  String excel(String[] titles , List<Orders> objecs, String tomcates,String pasres){

    String s = StringUtil.splicts(tomcates);
    // Excel 的表头
//    String[] title = {"id","name","sex"};
    ...
    String urs ="";
    if(pasres.equals("listAll")){
      //追加数据
       urs = "listAll"+new Date().getTime();
      for (int j = 1; j < objecs.size()+1; j++) {
        Orders ordes = objecs.get(j-1);
        HSSFRow nextrow = sheet.createRow(j);
        HSSFCell cell2 = nextrow.createCell(0);
        cell2.setCellValue(ordes.getType_name());
        cell2 = nextrow.createCell(1);
        cell2.setCellValue(ordes.getNumbers());

      }
    }
    if(pasres.equals("ListPolice")){
      //追加数据
      urs = "ListPolice"+new Date().getTime();
      for (int j = 1; j < objecs.size()+1; j++) {
        Orders ordes = objecs.get(j-1);
        HSSFRow nextrow = sheet.createRow(j);
        HSSFCell cell2 = nextrow.createCell(0);
        ...
        cell2 = nextrow.createCell(2);
        String starus = "";
        if(ordes.getStatus().equals("1")){
          starus = "库存报警";
        }else {
          starus = "正常";
        }
        cell2.setCellValue(starus);

      }
    }

    if(pasres.equals("listHistory")){
      //追加数据
      urs = "listHistory"+new Date().getTime();
      for (int j = 1; j < objecs.size()+1; j++) {
        Orders ordes = objecs.get(j-1);
        HSSFRow nextrow = sheet.createRow(j);
        HSSFCell cell2 = nextrow.createCell(0);
        cell2.setCellValue(ordes.getId());
        cell2 = nextrow.createCell(1);
        String starus = "";
        if(ordes.getOrderType().equals("1")){
          starus = "医院内部采购";
        }else {
          starus = "医院外部采购";
        }
        ...
        cell2.setCellValue(ordes.getNumber());
        cell2 = nextrow.createCell(4);

        cell2.setCellValue(DateUtil.formatDate(ordes.getCreateTime()));

      }
    }


    if(pasres.equals("ListDistribution") || pasres.equals("ListDepartment") || pasres.equals("list_history")
            || pasres.equals("List_purchasingPlan") ){
      //追加数据
      urs = pasres+""+new Date().getTime();
      for (int j = 1; j < objecs.size()+1; j++) {
        Orders ordes = objecs.get(j-1);
        HSSFRow nextrow = sheet.createRow(j);
        HSSFCell cell2 = nextrow.createCell(0);
    ...
    if(pasres.equals("list") ){
      ...
      for (int j = 1; j < objecs.size()+1; j++) {
        ...
        cell2.setCellValue(ordes.getDepartment_name());
        cell2 = nextrow.createCell(3);
...
//
//        cell2.setCellValue(ordes.getTime());
        cell2 = nextrow.createCell(5);
...
//    // 物资采购计划
//    if(pasres.equals("List_purchasingPlan") ){
//      //追加数据
//      urs = pasres+""+new Date().getTime();
//      for (int j = 1; j < objecs.size()+1; j++) {
//        Orders ordes = objecs.get(j-1);
