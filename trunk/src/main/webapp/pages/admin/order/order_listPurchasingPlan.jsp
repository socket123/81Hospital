...
<div id="contentwrapper" class="contentwrapper">
	...
	<div class="dataTables_wrapper" id="dyntable2_wrapper">
		...
		<a class="btn btn_orange btn_search radius50" onclick="excels('order','List_purchasingPlan')"><span>导出</span></a>
		<%--<a class="btn btn_orange btn_search radius50" onclick="searchFormListDepartment('order','List_purchasingPlan')"><span>搜索</span></a>--%>
		<div class="dataTables_filter" id="dyntable2_filter" style="width: 30%;">
			<label>种类:
				<select name="id" id="id" onchange="gradeChange('order','List_purchasingPlan')" style="margin: -7px 0 0 0;">
					<option value="">==全部==</option>
					<c:forEach items="${typelist}" var="typelist" varStatus="vars">
						<option value="${typelist.id}" <c:if test="${typelist.id == type.id}" > selected</c:if>   >${typelist.typeName}</option>
					</c:forEach>
				</select>
			</label>
		</div>
...
$('#page-jq').jqPaginator({
    ...
    onPageChange:function(num,type){
    	if(type == 'change'){
            ListDepartment('order','currentIndex='+num+'&'+$('#searchForm').serialize(),'List_purchasingPlan');
    	}
    }
});
