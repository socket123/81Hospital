...
<div id="contentwrapper" class="contentwrapper">
	...
	<div class="dataTables_wrapper" id="dyntable2_wrapper">
		...
		<div id="dyntable2_length" class="dataTables_length">
			<label><a class="btn btn2 btn_blue btn_bulb" onclick="add('department')"><span>新增</span></a></label>
		</div>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="stdtable" id="dyntable2">
			...
			<tbody>
				...
				<tr class="gradeX">
					...
					<td>${list.departmentIntroduce }</td>
					<td><fmt:formatDate value="${list.createTime }" pattern="yyyy-mm-dd HH:MM:ss"></fmt:formatDate></td>
					<td class="center">
					<input type="hidden" name="id" value="${list.id }">

							<a class="btn btn_flag" onclick="edit('department','${list.id }')"><span>编辑</span></a>

							<a class="btn btn_trash" onclick="del('department','${list.id }')"><span>删除</span></a>
...
$('#page-jq').jqPaginator({
    ...
    onPageChange:function(num,type){
    	if(type == 'change'){
            ListDepartment('department','currentIndex='+num+'&'+$('#searchForm').serialize(),"list");
    	}
    }
});
</script>